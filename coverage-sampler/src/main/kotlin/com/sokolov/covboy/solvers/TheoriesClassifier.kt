...
class TheoriesClassifier(private val prover: IProver) : FormulaVisitor<Int> {
    ...
    private fun Formula.checkType() {
        val type = prover.context.formulaManager.getFormulaType(this)
    ...
    override fun visitFunction(f: Formula, args: List<Formula>, declaration: FunctionDeclaration<*>): Int {
        ...
        args.forEach { arg ->
            val argLevel = prover.context.formulaManager.visit(arg, this)
            if (argLevel >= 1)
                numNonConstantArgs++
        ...
        if (
            ...
            if (declaration.type.isBooleanType) {
                if (
                    ...
                    args.forEach { arg ->
                        val type = prover.context.formulaManager.getFormulaType(arg)
